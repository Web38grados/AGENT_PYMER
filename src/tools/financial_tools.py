...
def obtener_datos_financieros(user_id: str) -> Dict[str, List[Dict[str, Any]]]:
    """Obtiene los datos financieros y de costos indirectos del usuario desde Firestore."""
    colecciones = [
        "financial_settings",
...
def calcular_punto_equilibrio(user_id: str, precio_venta: float, costo_variable_unitario: float) -> Dict[str, Any]:
    """Calcula el punto de equilibrio del negocio."""
    try:
        datos = obtener_datos_financieros(user_id)
...
def analizar_rentabilidad(user_id: str, ventas_mensuales: float, costo_ventas: float) -> Dict[str, Any]:
